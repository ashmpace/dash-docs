...
layout = html.Div([dcc.Markdown('''
  ...
  creating your own Dash component in React and JavaScript, even if you have never programmed in those languages before.

  [Let us know how it went by commenting in this thread](https://github.com/plotly/dash-docs/issues/194).
  ...
  ##### React
  React is a JavaScript library for building user interfaces, written and maintained by Facebook. It brings the power of reactive, declarative programming to the world of front-end development. 
  ...
  It's important to realize that React is just JavaScript. React is not a language on its own, nor is it a domain-specific framework
  ...
  Node is handy when developing, even when you intend to run the code in the browser.

  ##### NPM
  NPM stands for "Node Package Manager," and it is used to install packages and run scripts. Besides being a package manager 
  (like `pip` for Python), `npm` also allows you to perform tasks such as creating a project for you (`npm init`), 
  starting up a project (`npm start`), or firing custom scripts (`npm run custom-script`). These scripts are defined in a `package.json` file, which is a part of every NPM project. The `package.json` file holds your `requirements` and `devRequirements`, which can be installed using `npm install`, the same way `pip` has a `requirements.txt`
